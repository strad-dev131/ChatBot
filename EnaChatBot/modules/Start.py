...
@EnaChatBot.on_message(filters.new_chat_members)
        ...
        for member in message.new_chat_members:
            ...
            if member.id == EnaChatBot.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await EnaChatBot.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await EnaChatBot.download_media(
                ...
                count = await EnaChatBot.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
...
@EnaChatBot.on_cmd(["start", "aistart"])
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await EnaChatBot.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(EnaChatBot.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await EnaChatBot.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@EnaChatBot.on_cmd("help")
async def help(client: EnaChatBot, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{EnaChatBot.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve  αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||** ⋆ʟᴏᴠᴇ ᴡɪᴛʜ⋆ [ʏᴏ ᴍʏsᴛᴇʀɪᴏᴜs](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@EnaChatBot.on_message(filters.command("stats"))
...
@EnaChatBot.on_message(filters.command(["broadcast", "gcast"]))
async def broadcast_message(client, message):
    global IS_BROADCASTING

    user_id = message.from_user.id
    if not (user_id == int(OWNER_ID) or user_id in EXTRA_BROADCASTERS):
        return await message.reply_text("You don't have permission to use this command on this bot.")
        ...
        try:
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await EnaChatBot.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await EnaChatBot.send_message(
            ...
            if flags.get("-user", False):
                ...
                for user in users:
                    ...
                    try:
                        if broadcast_type == "reply":
                            await EnaChatBot.forward_messages(
